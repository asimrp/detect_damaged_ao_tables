...
drop table if exists aocsseg_count;
create table aocsseg_count as
  (select a.gp_segment_id as segid, a.relid, a.segrelid,
  	  (select count(segno) from gp_toolkit.__gp_aocsseg(a.relid)
	   where physical_segno >= 0 and physical_segno < 128) as aoseg_count
...
-- discrepancy segment file counts recorded in aoseg vs persistent
-- table.
select p.segid, a.relid::regclass, a.segrelid::regclass, a.aoseg_count, p.pt_count
  from aocsseg_count a full outer join pt_ao_column_segfile_count p
  on a.segid = p.segid and a.relid = p.relid
  where p.pt_count is null or a.aoseg_count is null or p.pt_count - a.aoseg_count > 1;
